...
- **MinGW 第三方库**：当前 `Dependence/lib` 中的 GLFW 为 **MSVC 格式（`.lib`）**。volk 由 Vulkan SDK 源码（`Include/Volk/volk.c`）随项目编译，不依赖 SDK 预编译的 `Lib/volk*.lib`。MinGW 配置可用于代码分析与部分编译流程；若需 MinGW 完整链接通过，需另行提供 MinGW 兼容的 GLFW 或从源码编译。
